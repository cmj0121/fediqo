...
migration 005 把 001 丟掉的東西留下來。附件當初只存了一個位址，完全沒說那是什麼東西——而對影片來說，
那個位址是**預覽圖**，真正的檔案從來沒被要過。內容警告直接被丟掉，每則貼文底下那三個數字也是。

| 表／欄位                              | 一列說的是什麼                                              |
| ------------------------------------- | ----------------------------------------------------------- |
| `post_media`                          | `merge_key` 的第 `position` 個附件是某個 `kind`，檔案在 `url`，預覽在 `preview_url`，作者的描述是 `alt` |
| `posts.sensitive`                     | 來源說了附件要遮著送過來                                     |
| `posts.spoiler_text`                  | 作者擺在正文前面的那一行                                     |
| `posts.replies_count` 與旁邊那兩欄    | 我們最後一次被告知時，這則貼文被怎麼對待                     |
| `posts.application` 與 `application_url` | 這則是用什麼寫的，如果伺服器有說（006）                  |

**這些全部不回填**，而且程式碼是照這個事實寫的，不是把它藏起來。005 之前的附件回來時 kind 是 `unknown`、
只有預覽沒有檔案——那正是它為真的部分。`sensitive`、`spoiler_text` 和三個數字對「從來沒被告知」的貼文都是
NULL，這跟 `0` 和 `''` 是不同的事實：畫面只能遮它真的被要求遮的東西，也永遠不能畫一個自己沒有的數字。

`application` 絕大多數時候是 NULL，而那是伺服器的作法：Mastodon 只會說自己站上寫的貼文用了什麼，
對聯邦過來的一律不提。沒有值的那一列就什麼都不畫——絕不寫成「來自未知的 app」，那是替別人的客戶端
捏造事實。

數字在每次由 authority 交出來時寫入，而且**永遠不動 `updated_at`**：有人收藏一則貼文，不等於作者改了它，
而 `updated_at` 只有一個意思。沒送數字的伺服器不會把已知的數字洗掉，因為 `coalesce` 會留住別人說過的。

`post_media` 是寫在 `posts.media_urls` **旁邊**，不是取代它：001 凍結的那一欄仍然為真，新的東西讀新的表。

## 討論串是貼文抵達的第四種方式

打開一則貼文時，會去問對它有最終發言權的那台伺服器（永遠不是轉載它的那台，跟 `reconcile` 同一條規則）
討論串。回來的是貼文，而貼文一律要記下它是透過哪個來源抵達的，所以 `thread` 是 `feeds` 裡與三個基底來源
並列的一列。沒有任何範本提供它、也沒有分頁：它是「這則貼文怎麼來的」，不是一個可以去讀的地方。
...
伺服器不再交出來的貼文是做記號，不是移除：`posts.deleted_at` 設一次、永不改寫，和其他每一個網路欄位一樣。
時間軸用 `WHERE deleted_at IS NULL` 略過它，而 partial index `posts_deleted` 找得到做了記號的列，不必掃
其餘的。

### 這個記號實際上在說什麼

不是「作者刪掉了它」。**它說的是：對這則貼文最有話語權的那台伺服器，不再把它交出來了** —— 這是刻意收窄
過的說法。

2026-08-25 對 mastodon.social 實測：它不肯給的貼文回的是 **404**，不是 410 —— 所以一個只等墓碑的儲存層，
永遠等不到。而一個沒有登入的請求收到的 404，同時涵蓋三件不同的事：貼文被撤下、貼文的能見度後來收窄了、
以及某個帳號把我們封鎖了。回應裡沒有任何東西能把它們分開，所以這個記號也不假裝分得開。三者共通、而且讀者
無論如何都會經歷的，就是那則貼文從那裡再也拿不到了。所以 404 和 410 被讀成同一個答案。

### 它是怎麼被寫下來的

光是「不在」永遠不夠。當一台伺服器交來一頁，這個儲存層裡**由那台伺服器**交來、落在那一頁所涵蓋的那段時間
之內、而又不在那一頁裡的貼文，成為**嫌疑** —— 那是一個問題，永遠不是一個判決。伺服器會把讀者封鎖的帳號和
被過濾器藏起的貼文，從它已經挑好的範圍裡拿掉，所以一頁本來就常常漏掉好端端還在的貼文；而打開一個過濾器，
可以一次漏掉整整一頁。

每一頁時間軸都算數，刷新的那一頁和往下讀的那一頁一樣：是誰要來的，並不改變一頁是什麼的證據；而且**最新的
那一段正是往下讀永遠不會再經過的一段** —— 貼文剛發出沒多久就被撤下，位置就正好在那裡。trending 清單不是這
種頁，什麼問題都不產生：它是伺服器策展出來的一張快照，一則貼文不在上面，只代表伺服器不認為它正在竄升，而
那不是「這則貼文還在不在」的說法。

那段時間是那一頁**帶回來的** —— 它自己的最新與最舊 —— 不是我們要求的那一段。要了四十則、回來三則的一頁，
只為那三則說話，再往外一步都不算。而**一頁空的，什麼時間都沒涵蓋**，也就什麼都沒漏掉：那是伺服器在說它沒
有更早的了；把它讀成「一頁漏掉了全部」，等於讀者每次讀到底，整條時間軸就被懷疑一次。

一則嫌疑只能靠指名去問那一則來了結，而且只有答案能了結它：

| 答案       | 會發生什麼                                 |
| ---------- | ------------------------------------------ |
| 404 或 410 | 設下 `deleted_at`                          |
| 200        | 撤掉這個嫌疑 —— 貼文還在                   |
| 401 或 403 | 什麼都不決定，也不啟動等待 —— 它回應了     |
| 其他任何   | 什麼都不決定；改天再問一次                 |

「拒絕」不是「沉默」，而這個分別有兩層意義。一台伺服器拒絕把某則貼文給陌生人看，它是**當場回應**了，所以
不該因此進入 backoff —— 而且那個等待以端點為鍵、和被選的伺服器共用，把拒絕當成沉默，會讓一台伺服器只因為
回絕了一個匿名的提問，就連自己已登入的時間軸都被跳過。但一個持續的拒絕仍然是一個持續的答案：連續三次之
後，這個問題會被**擱置** —— 不做記號、不遺忘，只是不再佔著一輪八個名額裡的一個。重新啟動 app 就會再問一
次，因為這些狀態都活不過一次執行。

被問的是 `posts.authority_url`，也就是貼文自己的正規位址所指名的那台伺服器 —— 永遠不是
`posts.source_url`，不是碰巧把它交過來的那一台。一台不再轉載別人貼文的中繼，並沒有對「那則貼文是否還存
在」說過任何話，所以它的沉默不是證據。沒有 `authority_url` 的貼文，或正規位址不是這個版本問得動的形狀的貼
文，根本沒有人能回答，因此永遠不會被做記號。

轉推繼承它所承載那則貼文的權威方，所以問一則轉推，等於問原文是否還在。原文沒了，轉推就跟著沒了 —— 這是
對的，一則沒有人讀得到的貼文，轉推它也顯示不出任何東西。反過來，轉推者自己收回而原文仍在，答案仍然是
「還在」，那一列就留著。要注意到那件事是另一個問題，這裡不問。

確認是有上限的：一輪最多問八則，而還在 backoff 等待中的權威方完全不問，所以一台本來就在苦撐的伺服器，不會
接著被一則一則的請求砸。剩下的等下一輪。它永遠不會被丟掉，也永遠不會被當成已經查過。
