...
007 帶進來三種答案，分開存在三個地方，因為它們屬於三種不同的東西。

| 答案                       | 存在哪              | 鍵                          | 誰被告知           |
| -------------------------- | ------------------- | --------------------------- | ------------------ |
| 收藏、轉貼、加書籤         | `post_marks`        | `(merge_key, author_id)`    | 一台伺服器         |
| 留下                       | `posts.kept_at`     | 這則貼文                    | 沒有人             |
| 靜音                       | `mutes`             | `(kind, value, server_url)` | 一台、兩邊，或都沒有 |

`post_marks` 用兩個東西當鍵，理由和 `post_origins` 一樣。`posts` 對一則貼文只有一列，不管有幾台伺服器帶
它過來；而「收藏了」並不是關於這則貼文的事實，它是關於**某個帳號與它的關係**的事實 —— 一個在兩台伺服器
都登入的讀者，可以用其中一個帳號收藏、另一個沒有。放在 `posts` 上，兩個帳號會互相覆蓋。

NULL 是「沒被告知」，不是 `false` —— 和 005 之後的 `sensitive` 同一條規則。這個 app 幾乎所有讀取都是以陌
生人身分進行的，而伺服器回答陌生人時，不會說你哪個帳號收藏過這則。畫面可以為它畫一顆空心的星，但不可以
說「未收藏」。

取消時寫回 NULL，而不是新增一列「在某時取消」。取消之後就沒有什麼還需要記得的了，而記住某人取消收藏過什
麼，是一份閱讀紀錄。

`kept_at` 是欄位不是表，也不以帳號為鍵，因為「留下」既不是帳號的事實也不是伺服器的事實 —— 它是**這台裝
置**的。沒有任何伺服器被告知，這正是它和 `bookmarked_at` 的全部差別。它的用途是輪替（設了就不被輪替
掉），而 007 沒有實作輪替；這一欄只是先給答案一個住處。

`mutes` 是一條規則一列，而不是一個目標一列加一個旗標。`server_url` 為 NULL 是這台裝置自己的規則，不告訴
任何人；寫了伺服器的那一列，是那台伺服器代為執行的。兩者可以同時對同一個作者成立，而且**必須**可以：這
個儲存層的承諾是它永遠說得出「是你的規則擋的，還是伺服器擋的」，一列加一個旗標說不出兩件事。

### 動作的代價

貼文自己所在的伺服器，本來是收藏最自然的去處，但幾乎永遠不可得：這個 app 讀的伺服器裡，有很多是沒有人在
上面有帳號的。所以動作是透過**讀者自己的**伺服器發出的，而那台伺服器必須先知道這則貼文，才能被問任何關
於它的事。

在那裡找到它是兩個問句，而且經常只需要第一個。`GET /api/v2/search` 帶 `resolve=false` 問的是伺服器**已
經**有什麼；只有在那個問句空手而回時，才送出 `resolve=true` —— 而那一次請求，就是要伺服器去把貼文抓回
來。它是這個 app 唯一一種「告訴別人你在讀什麼」的請求，所以除非讀者說可以，否則不送；而它花了什麼代價會
回傳，讓畫面說出來。
...
`static_url` 是會動的那張的靜止版。它被留下來，而且被畫出來的就是它：一行文字裡的圖不會動，會動的那份只會變成用影片
的尺寸抓下來的第一個影格 —— 而要求少一點動態的讀者，得到的是同一張圖，而不是程式裡的第二條路。
