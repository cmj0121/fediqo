...
空的啟動會打開帳號頁。加入一個尚未登入的 Mastodon 主機或一個 Discuz 論壇當來源——從目錄挑，或自己輸入主機名稱。
它會說出協定的名字；這個工作階段只有這兩種能加入。公開與趨勢貼文，以及論壇的主題，進到 store。
「全部」與「趨勢」是對那個 store 的 query。時間軸、通知與發文在還沒有東西可做之前會停用。
重新啟動之後，store 還在，論壇的登入也還在：它的 cookie 留在這台裝置上。
「清除」會拿掉一個伺服器的快取副本與登入，但留下它的貼文。沒有 OAuth。

這份 checkout 還沒有 release tag：mascot，以及離開後仍留在這台裝置上的 Mastodon 與論壇來源。
