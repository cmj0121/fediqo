...
/// went red on a test that was right. Sixty seconds is past any honest wait, so the only run that
/// reaches the end is one whose gate nobody was ever going to open, and that run says so rather
/// than passing on whatever the gate let through. `.timeLimit` does not rescue a task parked on a
