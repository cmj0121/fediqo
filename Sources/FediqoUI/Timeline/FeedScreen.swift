...
struct FeedScreen: View {
    ...
    var body: some View {
        ...
        return ScrollViewReader { proxy in
            ...
            // The one threshold S9 allows, asked once for the whole list. `Space.gap` is what
            // `body(for:)` puts around its rows, and it is not width a row has (#118).
            .fediqoMeasuresRows(rowsInsetBy: Space.gap)
